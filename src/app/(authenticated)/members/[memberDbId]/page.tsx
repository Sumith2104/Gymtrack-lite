...
export default async function MemberProfilePage({ params }: { params: Promise<{ memberDbId: string }> }) {
  const { memberDbId } = await params;
  const { data: member, error } = await getMemberById(memberDbId);
