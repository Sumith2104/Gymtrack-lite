...
import { flux } from '@/lib/flux/client'; // Use Flux Client
...
async function getSuperAdminTransporter(): Promise<{ transporter: Transporter; fromEmail: string } | null> {
  try {
    ...
    const superAdminSmtp = result.rows[0];

    if (!superAdminSmtp.smtp_host || !superAdminSmtp.smtp_port || !superAdminSmtp.smtp_username || !superAdminSmtp.smtp_pass) {
...
async function getTransporter(gymDatabaseId?: string | null): Promise<{ transporter: Transporter | null, fromEmail: string }> {
  ...
  try {
    const query = `
        ...
        `;
    const result = await flux.sql(query);
    const gymSmtp = result.rows?.[0];

    // If gym-specific settings are incomplete, fall back to the defaults
    if (!gymSmtp || !gymSmtp.app_host || !gymSmtp.port || !gymSmtp.app_email || !gymSmtp.app_pass) {
      return { transporter: defaultTransporter, fromEmail: defaultFromEmail };
    }
